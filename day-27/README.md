...
- Tkinter module has different layout managers that define how to position the widgets in the GUI program
- A widget will not be displayed unless we specify a layout manager
- Three layout managers:
  - pack() - pack each widget one on top of each other. Difficult to specify a precise position
    - `button.pack(side="left")`
  - place() - place a widget at a specified precise coordinate. Can provide x-y values
    - `button.place(x=0, y=0)`
  - grid() - place a widget in a grid system. Specify the row and column
    - `button.gride(row=2, column=0)`
- NOTE: pack() and grid() are not compatible with each other. Choose one or the other

### Day 27 project: Miles to Kilometer Converter
- File: 
```py
from tkinter import *
